...
public class VectorSample : MonoBehaviour
{
    ...
    //프로그램이 실행되는 동안 중앙 제어 장치(CPU)가 코드를 처리하기 위해 메모리가 명령어와 데이터들을 저장
    //하고 있어야 합니다.
    ...
    //   전역 변수(global) : 프로그램 어디서나 접근 가능한 변수(C#d에서는 전역 대신
    //   클래스  수준의 정적 변수를 사용합니다.)

    //   정적 변수(static) : static 키워드가 붙은 변수는 별도의 객체 생성 없이 클래스명.변수명
    //   으로 직접 접근하는 것이 가능합니다.
